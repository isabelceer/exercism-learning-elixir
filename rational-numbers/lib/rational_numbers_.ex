defmodule RationalNumbers do
  ...
  def divide_by({num_a, den_a}, {num_b, den_b}) when num_b !== 0 do
    {num_a * den_b, den_a * num_b} |> reduce()
  end
  ...
  {Kernel.abs(num_a), Kernel.abs(den_a)} |> reduce()
  end
...
def pow_rational({num_a, den_a}, n) when n < 0 do
    {Integer.pow(den_a, Kernel.abs(n)), Integer.pow(num_a, Kernel.abs(n))} |> reduce()
 end   
  
def pow_rational({num_a, den_a}, n) when n > 0 do
    {Integer.pow(num_a, n), Integer.pow(den_a, n)} |> reduce()
  end
...
@doc """
  ...
  def pow_real(x, {num_n, den_n}) do 
:math.pow(x, num_n / den_n)
end
